...
export const StealOverlay: React.FC = () => {
  ...
  if (!state || state.gamePhase !== 'robber' || !state.victims || state.victims.length === 0) return null;

  // Only show to the current player
    ...
    <div className="absolute inset-0 z-40 bg-black/60 backdrop-blur-md flex items-center justify-center">
      ...
      >
        <div className="text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-accent/20 text-accent-dark rounded-full mb-4">
            <Hand size={32} />
          </div>
          ...
          {state.victims.map((victimId) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-4 p-5 bg-gray-50 hover:bg-accent hover:text-text-dark rounded-2xl border border-gray-100 transition-all group"
              >
